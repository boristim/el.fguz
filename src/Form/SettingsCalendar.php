        ...
        if (count($result)) {
            foreach ($result as $nid) {
                ...
                if (!array_key_exists($cdt->format('d.m.Y H:i'), $valuesC)) {
                    $message = 'Удалена регистрация ' . $cdt->format('d.m.Y H:i') . ' ' . $node->get('field_fio')->getValue()[0]['value'] . ' ' . $node->get('field_tel')->getValue(
                        )[0]['value'] . ' ' . $node->get('field_description')->getValue()[0]['value'] . ' ';
